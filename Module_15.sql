...
-- Все объекты создаются в схеме "Zelent"

-- Установка search_path для работы в схеме "Zelent"
SET search_path TO "Zelent", public;

-- Задание 1. Скалярная функция — расчёт OEE (простое)
CREATE OR REPLACE FUNCTION "Zelent".calc_oee(
...
SELECT
    "Zelent".calc_oee(10, 12, 80, 100) AS test1,  -- ожидаемый результат ~66.7
    "Zelent".calc_oee(12, 12, 100, 100) AS test2, -- ожидаемый результат 100.0
    "Zelent".calc_oee(8, 12, 0, 100) AS test3,    -- ожидаемый результат 0.0
    "Zelent".calc_oee(8, 0, 100, 100) AS test4;   -- ожидаемый результат NULL
...
CREATE OR REPLACE FUNCTION "Zelent".classify_downtime(p_duration_min INT)
RETURNS VARCHAR
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
WITH downtime_data AS (
    SELECT
        "Zelent".classify_downtime(fd.duration_min::INT) AS category,
        fd.duration_min
...
CREATE OR REPLACE FUNCTION "Zelent".get_equipment_summary(
    p_equipment_id INT,
    p_date_from INT,
    p_date_to INT
)
...
SELECT * FROM "Zelent".get_equipment_summary(1, 20240101, 20240131) LIMIT 10;
...
CROSS JOIN LATERAL "Zelent".get_equipment_summary(e.equipment_id, 20240101, 20240131) s
WHERE e.mine_id = 1
ORDER BY e.equipment_name, s.report_date
LIMIT 20;

-- Задание 4. Функция с дефолтными параметрами — гибкий фильтр (среднее)
CREATE OR REPLACE FUNCTION "Zelent".get_production_filtered(
    p_date_from INT,
    p_date_to INT,
...
-- Все данные
SELECT * FROM "Zelent".get_production_filtered(20240101, 20240131) LIMIT 10;

-- Только шахта 1
SELECT * FROM "Zelent".get_production_filtered(20240101, 20240131, p_mine_id := 1);

-- Шахта 1, дневная смена (предполагая, что shift_id = 1 - дневная смена)
SELECT * FROM "Zelent".get_production_filtered(20240101, 20240131, 1, 1);

-- Задание 5. Процедура с транзакциями — архивация данных (среднее)
-- Создание архивной таблицы в схеме "Zelent"
CREATE TABLE IF NOT EXISTS "Zelent".archive_telemetry (LIKE fact_equipment_telemetry INCLUDING ALL);

CREATE OR REPLACE PROCEDURE "Zelent".archive_old_telemetry(
...
BEGIN
    ...
    INSERT INTO "Zelent".archive_telemetry
...
-- CALL "Zelent".archive_old_telemetry(20240101, NULL, NULL);

-- Проверка данных в архивной таблице
-- SELECT COUNT(*) FROM "Zelent".archive_telemetry;

-- Задание 6. Динамический SQL — универсальный счётчик (среднее)
CREATE OR REPLACE FUNCTION "Zelent".count_fact_records(
    p_table_name TEXT,
    p_date_from INT,
    p_date_to INT
)
...
SELECT "Zelent".count_fact_records('fact_production', 20240101, 20240131) AS production_count;
SELECT "Zelent".count_fact_records('fact_equipment_downtime', 20240101, 20240131) AS downtime_count;

-- Этот вызов должен вызвать ошибку:
-- SELECT "Zelent".count_fact_records('dim_mine', 20240101, 20240131);

-- Задание 7. Динамический SQL — построитель отчётов (сложное)
CREATE OR REPLACE FUNCTION "Zelent".build_production_report(
    p_group_by TEXT,
    p_date_from INT,
    p_date_to INT,
...
SELECT * FROM "Zelent".build_production_report('mine', 20240101, 20240131);
SELECT * FROM "Zelent".build_production_report('shift', 20240101, 20240131);
SELECT * FROM "Zelent".build_production_report('equipment_type', 20240101, 20240131, 'dimension_name ASC');

-- Задание 8. Комплексная процедура — ежедневная загрузка данных (сложное)
-- Создание staging-таблиц в схеме "Zelent"
CREATE TABLE IF NOT EXISTS "Zelent".staging_daily_production (
    date_id INT,
    equipment_id INT,
    shift_id INT,
    operator_id INT,
...
CREATE TABLE IF NOT EXISTS "Zelent".staging_rejected (
    date_id INT,
    equipment_id INT,
    shift_id INT,
    operator_id INT,
...
CREATE OR REPLACE PROCEDURE "Zelent".process_daily_production(
    p_date_id INT,
    OUT p_validated INT,
    OUT p_rejected INT,
    OUT p_loaded INT
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM "Zelent".staging_daily_production WHERE date_id = p_date_id) THEN
    ...
    INSERT INTO "Zelent".staging_rejected (
    ...
    FROM "Zelent".staging_daily_production s
...
/*
INSERT INTO "Zelent".staging_daily_production (date_id, equipment_id, shift_id, operator_id, tons_mined, trips_count, operating_hours, fuel_consumed_l)
VALUES
...
-- Вызов процедуры
CALL "Zelent".process_daily_production(20240115, NULL, NULL, NULL);

-- Проверка результатов
SELECT * FROM fact_production WHERE date_id = 20240115;
SELECT * FROM "Zelent".staging_rejected WHERE date_id = 20240115;
*/
